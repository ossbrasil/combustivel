...
$mes = $_SESSION['relatorio_carmes'];
$ano = $_SESSION['relatorio_carano'];
// $prefixo = $_SESSION['relatorio_carprefixo'];
$tipo = 0;
...
// Query 
$query = "SELECT DISTINCT(prefixo) FROM `gestaocmv` WHERE data_req LIKE '%$data_req%' ORDER BY prefixo";
$result = mysqli_query($conn, $query);

while ($consulta_result = mysqli_fetch_assoc($result)) {
    $prefixo = $consulta_result['prefixo'];
    ...
    $mpdf->WriteHTML('
            <div>
                <div id="titulo" style="display: flex;">
                    <div>
                        <img src="../imagens/resPcarro_head.jpg" alt="logo">
                    </div>
                    <table style="width:100%;">
                        <tr>
                            <th>Mês: ' . $mes_a . '</th>
                            <th>Prefixo: ' . $prefixo . '</th>
                        </tr>
                    </table>
                </div>
            </div>');
    ...
    for ($i = 0; $i <= $diasDoMes; $i++) {
        ...
        if ($i != 0) {
            ...
            $queryvg = "SELECT servicos, kmfim, kminicio FROM `gestaocmv` WHERE data_req = '$data' AND polo = '$polo_vg' AND periodo = '0' AND prefixo = '$prefixo'";
            // $queryvg = "SELECT * FROM `gestaocmv` WHERE data_req = '$data' AND polo = '1' AND periodo = 'dia'";
            $resultvg = mysqli_query($conn, $queryvg);
            $row_cnt = $resultvg->num_rows;

            if ($row_cnt > 0) {
                while ($consulta_result = mysqli_fetch_assoc($resultvg)) {
            ...
            $queryvg = "SELECT servicos, kmfim, kminicio FROM `gestaocmv` WHERE data_req = '$data' AND polo = '$polo_vg' AND periodo = '1' AND prefixo = '$prefixo'";
            ...
            $queryvm = "SELECT servicos, kmfim, kminicio FROM `gestaocmv` WHERE data_req = '$data' AND polo = '$polo_vm' AND periodo = '0' AND prefixo = '$prefixo'";
            // $queryvm = "SELECT * FROM `gestaocmv` WHERE data_req = '$data' AND polo = '3' AND periodo = 'dia'";
            $resultvm = mysqli_query($conn, $queryvm);
            $row_cnt = $resultvm->num_rows;

            if ($row_cnt > 0) {
                while ($consulta_result = mysqli_fetch_assoc($resultvm)) {
                    $serv = $consulta_result['servicos'];
            ...
            $queryvm = "SELECT servicos, kmfim, kminicio FROM `gestaocmv` WHERE data_req = '$data' AND polo = '$polo_vm' AND periodo = '1' AND prefixo = '$prefixo'";
...
//Use a flag 'D' para download e a 'i' pra visualizar na web
$mpdf->Output('Relatorio_todos_carros.pdf', 'D');
